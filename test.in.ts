...
export const schémaStructureNomsDispositifs: JSONSchemaType<
  structureNomsDispositifs
> = {
  type: "object",
...
export class Constellation {
            ...
            try {
              ...
              if (messageJSON["port"]) {
                const erreur = new Error(
                  `Ce compte est déjà ouvert en Constellation, et le serveur local est disponible sur le port ${
                    messageJSON["port"]
                  }. Vous pouvez soit vous connecter sur ce port, soit fermer les instances de Constellation qui ouvertes et puis vous ressayer.`,
                );
  ...
  >({
    ...
    const { bd: nouvelleBdDic, fOublier: fOublierNouvelle } = await this
      .ouvrirBdTypée({
        id: idNouvelleBdDic,
        type: "keyvalue",
        schéma,
